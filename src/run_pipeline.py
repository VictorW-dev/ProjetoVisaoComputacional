...
def process_video(video_name, fps, max_people):
    input_video_path = f"data/raw/{video_name}.mp4"
    ...
    subprocess.run(
        f"python src/1_extract_frames.py --input_video {input_video_path} --output_dir data/processed --fps {fps}",
        shell=True
    )
...
def run_pipeline(train_violence, train_nonviolence, test_violence=None, test_nonviolence=None, model="lstm", fps=10, max_people=3):
    ...
    if test_violence and test_nonviolence:
        ...
        eval_cmd = f"python src/5_evaluate_model.py --model {model} --model_path {model_path} --violence {' '.join(test_v)} --nonviolence {' '.join(test_nv)} --max_people {max_people}"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_violence", nargs="+")
    parser.add_argument("--test_nonviolence", nargs="+")
